	...
	<div id="blog">
		<div id="header">
			...
			<!-- sec:authorize access="hasRole('ADMIN')"-->
				<h2 style="color:blue">Admin Page: The list of articles!</h2>
			<!--  /sec:authorize-->
				...
				<div id="main">
					...
					<div id="article">
						...
						<div id="footer" align="right">
							...
							<c:set   value="admin" var="administrator"/>
							<c:if test="${pageContext.request.userPrincipal.name == administrator and article.enabled == '0'}">
	                        	<td><a href="<c:url value='/validateArticle'><c:param name='title' value="${article.title}" /></c:url>">Validate Article</a></td>
	                        </c:if>	
	                        <c:if test="${article.enabled == '1'}">
								...
								<c:if test="${title==titleComm}">
									...
									<div id="comment">
										<table>
											...
											<tr><td>
											<c:if test="${comment.enabled == '0'}">
	                        					<a href="<c:url value='/validateComment'><c:param name='commentId' value="${comment.commentId}" /></c:url>">Validate Comment</a>
	       					                </c:if>	
					                        <c:if test="${pageContext.request.userPrincipal.name == comment.username}">	
					                        	<c:if test="${comment.enabled == '1'}">
					                        	<a href="<c:url value='/editComment'><c:param name='commentId' value="${comment.commentId}" /></c:url>">Edit Comment</a>
					                            </c:if>
					                            <a href="<c:url value='/deleteComment'><c:param name='commentId' value="${comment.commentId}" /></c:url>">Delete Comment</a>
					                        </c:if>
					                        </td></tr>	
										</table>
									</div>					
